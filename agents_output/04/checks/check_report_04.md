...
## 增量验收（2026-07-16 · P4 AI 相机启动预览）

| 条目 | 结论 | 证据 |
|---|---|---|
| 本页会话仅播放一次 | ✅ | `App` 持有不落盘的 `cameraIntroPlayedRef`；P4 首次激活即标记，离开再返回不重播 |
| 动画覆盖层与兼容 | ✅ | WebM 优先 + MP4 回退，全屏黑底 `contain`，播放时 `data-swipe-lock` 阻断误滑 |
| 退出与降级 | ✅ | 结束/跳过/媒体错误/自动播放失败/9 秒超时统一淡出；reduced-motion 直接进入 P4 |
| 摄像头权限边界 | ✅ | 预览层仅播放本地视频；`getUserMedia` 仍只由原有相机按钮触发 |
| 自动化回归 | ✅ | `npm test` 全绿；真浏览器确认 8s WebM 自然结束、跳过淡出、返回不重播、reduced-motion 0 视频请求、预览期 `getUserMedia` 0 调用 |
