...
- 路由必须使用真实 pedestrian/foot profile，不接受仅改 URL profile 名称的机动车数据集。
- 服务端校验 `duration / distance`，可信范围为每公里 10–16 分钟；越界响应按服务失败处理。
- 首选支持步行路网的公共服务；请求携带项目 UA，频率不超过 1 次/秒。
- 服务超时、5xx、无路线或速度不可信时，最多重试 2 次，再降级为起终点直线。
- 直线仅表达方向，不冒充可行走路线；界面同时展示机位库 `walk_steps` 文字步导。
- 平台内置地图若不能声明步行 profile，直接采用直线 + 文字步导，不展示伪步行时长。
