...
## P2 实时光影地图（v1.1 主升级 · 用户指定的主战场）

> **形态链（2026-07-13 定稿）**：**GL 版为主**（`light-map-gl.jsx`，MapLibre GL + OSM 实时矢量瓦片——完整道路网/真实建筑 3D 挤出/地名水系全真，光照方向=太阳方位角，拖=平移/双指=旋转缩放原生手势）→ 离线降级 **Three 自研版**（`light-map-3d.jsx`，预拉 OSM 建筑）→ **经典 2D**。演示起点=南方科技大学（大区赛场地，待校名最终确认）。
>
> **⚠️ 平台迁移预案（第一性原理 F2：最终必须能迁抖音AI平台，挂 AGENT_06 7.24 测绘）**：
> - **A 档**：平台支持自定义 Web 运行时/可执行组件 → MapLibre 方案直迁（引擎 BSD、瓦片 ODbL，合规）。
> - **B 档**：平台仅有内置地图组件 → 2D 地图 + 光位罗盘组件 + 太阳方位读数（光影叙事以罗盘+色彩承载）。
> - **C 档**：仅支持静态/低交互 → 本原型录屏内嵌为动效背景 + 点按式路线卡。
> - 三档"光在哪个方向"的叙事都成立；原型 GL 版同时充当 Figma/录屏素材源。
