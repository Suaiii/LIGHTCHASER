...
## 2026-07-14 · HERMES-03 光区提案 v5.0

- 新增三个默认关闭的 URL 模式：`lightZone=axis`、`spots`、`both`。太阳轴用 SunCalc 方位角，路线光位用当前瓦片建筑 footprint 与精确射线求交。
- 光位采样与 Three LOD 解耦。每个 camera generation 必须等当前瓦片 source 完整；779 个走廊 footprint 中仅 4 个命中太阳射线，650 cap 未触发。端到端计算 46–70 ms，只在 source 完成或交互停稳后执行。
- 太阳轴跟随视口中心，路线光位优先正式机位并补充视口内真实路线取样。路线亮芯截图亮度 217.0，高于见光点 200.7 和太阳轴 195.0。
- 回归：四模式 E2E 覆盖方位、屏内候选、partial data、移动/旋转 generation、移动中零计算和已日落状态；塔楼步进关闭/both 均为 1.1x（多轮最高 1.3x）；旋转回位 0.000%；全部 0 个 JS 错误；`npm test` 已包含几何单测并通过。
- 当前状态是待验收。还需未参与开发者做 3 秒测试，并由决策者选择 axis、spots 或 both；合并前删除落选方案。
