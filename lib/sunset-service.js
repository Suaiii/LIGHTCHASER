...
function buildRecommendation(cityKey, lat, lng, profile) {
  ...
  return {
    ...
    reason: profile.recommendationReason || poi.reasonHint,
  };
}

function getTips(profile, fallbackHint) {
  if (profile.shootingTips && profile.shootingTips.length > 0) {
...
function buildPayloadFromProfile({
  ...
  return {
    ...
    recommendation,
    shootingTips: getTips(profile, recommendation.reason),
    meta: {
      source,
      city: city.name,
...
async function buildSunsetPayload(query = {}) {
  ...
  const baseSunset = baseSunTimes.sunset || new Date(now.getTime() + 90 * 60 * 1000);
