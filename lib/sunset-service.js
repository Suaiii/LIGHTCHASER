...
const { getCityPreset, getNearestPoi, getNearbyPois, formatDistance } = require("./poi");
const {
  average,
...
function formatSunPosition(date, lat, lng) {
  const position = SunCalc.getPosition(date, lat, lng);
  const toDegrees = 180 / Math.PI;
  return {
    altitudeDeg: Number((position.altitude * toDegrees).toFixed(1)),
    azimuthDeg: Number(((position.azimuth * toDegrees + 180 + 360) % 360).toFixed(1)),
  };
}
...
function buildRecommendation(cityKey, lat, lng, profile) {
  ...
  const demoReason = isPrimaryDemoCity && profile.recommendationReason
  ...
  return {
    ...
    reason: `${poi.reasonHint}${demoReason}`,
  };
}

function buildNearbySpots(cityKey, lat, lng) {
  return getNearbyPois(cityKey, lat, lng, 4).map((poi) => ({
    name: poi.name,
    direction: poi.direction,
    distance: formatDistance(poi.distanceKm),
    coordinates: {
...
function buildPayloadFromProfile({
  ...
  const nearbySpots = buildNearbySpots(city.key, lat, lng);
  ...
  return {
    ...
    meta: {
      ...
      sun: {
        current: formatSunPosition(now, lat, lng),
        peak: formatSunPosition(peakTime, lat, lng),
      },
      debug,
    },
  };
