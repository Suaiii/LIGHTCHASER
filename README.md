...
**本项目按任务分发制运转**：所有"现在该干什么"集中在一张看板上。

1. **打开看板**：[`docs/hermes/README.md`](docs/hermes/README.md) —— 全景版图 + 可领取任务 + 流程规则
2. **领任务**：看板挑"待领"→ 读对应任务书 `docs/hermes/HERMES-XX-*.md`（自包含，读完即可开工，不需要问历史）→ 在对应 [issue](../../issues) 评论认领
3. **交付**：按任务书"交付方式"节走 `feat/*` 分支 → PR → 验收人跑 DoD

> 任务书里写死了：为什么做（对赛题的回答）／目标／二元验收标准／输入材料／红线／**占用文件**（防止和别人改同一个文件打架）。除任务书列出的占用文件外，一行都不要改。
...
| 想了解… | 读这个 |
|---|---|
| **现在该干什么 / 领任务** | **`docs/hermes/README.md`（看板，最高优先）** |
