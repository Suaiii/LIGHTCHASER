    ...
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, color: "rgba(255,255,255,0.92)", lineHeight: 1.5, marginBottom: 6 }}>
          ...
          <span style={{ color: "#ffd49a" }}> #静安寺</span>
        </div>
...
//   - 底部机位卡：距离 + 峰值时刻 + 左滑提示
// ─────────────────────────────────────────
function SceneSunsetCard({ score = 87, peak = "18:15" }) {
  const info = scoreInfo(score);
  const burst = skyColor(0.78);   // 峰值红
  const golden = skyColor(0.50);  // 橘
  const violet = skyColor(0.92);  // 紫
      ...
      <div style={{
        ...
        background: `
          linear-gradient(180deg,
            ${rgb(skyColor(0.20))} 0%,
            ${rgb(skyColor(0.45))} 30%,
            ${rgb(skyColor(0.65))} 55%,
            ${rgb(skyColor(0.78))} 72%,
            ${rgb(skyColor(0.90))} 88%,
            #1a0d18 100%)
        `,
      }} />
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 4, fontSize: 11, color: "rgba(255,255,255,0.7)" }}>
              <span><span style={{ color: "#ffd49a" }}>1.2</span>km</span>
...
// Scene 3 — 时间条 + 色卡 (产品灵魂)
// ─────────────────────────────────────────
function SceneTimeSlider() {
  const [t, setT] = useState(0.62);  // 默认在 18:02 日落
  const trackRef = useRef(null);
  const dragging = useRef(false);

  const c = skyColor(t);
  const cTop = skyColor(Math.max(0, t - 0.18));
  const cMid = c;
  const cBot = skyColor(Math.min(1, t + 0.15));
  ...
  const anchors = [
    ...
    { t: 0.33, label: "17:30", note: "黄金时刻" },
    { t: 0.62, label: "18:02", note: "日落" },
    { t: 0.78, label: "18:15", note: "峰值" },
