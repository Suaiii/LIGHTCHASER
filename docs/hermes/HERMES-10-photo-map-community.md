...
6. 亮度纪律（服从 D-c 三层排序，Leaflet 版实测口径见 DoD）；
...
4. 在 Leaflet 版地图（`subpanels.jsx` LightNavigationMap）实现照片气泡层 demo + **把独立「社区」列内容并入地图气泡**（colLabels/顶部 tabs 更新为三板块，触发 HERMES-09 B 包截图重拍）。"右滑看附近照片"出 2 个方案（Tweaks/URL 参数切换，默认关）：
...
- [ ] Phase 2 demo **组合态 3 秒测试**：金线+气泡**同屏**画面，向 ≥2 位未参与者提开放问句"这个画面在告诉你什么"，两人都能 3 秒内答出"附近有人拍到了什么/在哪"（记录原话进提案文档）。
- [ ] 时间筛选实测：切"今天/本周"气泡集合可见变化（种子数据须保证两档结果不同）。
- [ ] 气泡点开卡「去这里」真触发路线重算（selectedSpot 链路，非跳转假链接）。
- [ ] 亮度纪律（Leaflet 版两层口径）：截图取色器抽 5 点，金色路线亮芯 > 气泡边框/角标（照片内容不计入）；完整三层（金线>见光点>气泡）随 HERMES-11 GL 移植执行。
