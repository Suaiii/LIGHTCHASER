...
> - **B 设计包 🔻 砍**：保留保真元素清单 + 截图集**仅 1x 一套**（2x 砍）；**Figma 子项整体移出 Gate 0**——半天人力+需账号，全包最贵最不确定；7.24 要测 Figma 通道用**任意 Figma 链接**即可验通道。截图命名 **D-b 定案＝按现状四页 `feed/p1/p2/p3/p4`**（原型代码未重排，只能照拍现状；decisions.md"按重排推进"作废）。
...
- **受众**：**混合**——A/C/D/E 四包=AI 编码代理；~~B 包 Figma 制作=人~~（**Figma 已移出 Gate 0**，B 包剩余部分归 AI）
