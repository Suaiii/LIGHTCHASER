...
> - **B 设计包（🔁 7.20 晚负责人修正：Figma 恢复高优）**：保真元素清单保留；截图集**仅 1x 一套**（2x 砍）。**Figma 稿＝Gate 0 高优交付**——负责人裁定：平台可**直接导入 Figma**，已有稿能大幅节约平台内设计成本（早前"移出 Gate 0"的砍法作废）。路线维持已拍板的**工具转换+人工整理**（插件逐页导入→人工整理半天，整理优先级＝feed 卡→p2 地图→其余页），链接落 `figma_link.md`，**7.23 验"链接可访问"**；账号/人力意外时最后落点 7.24 晨。截图命名 **D-b 定案＝按现状四页 `feed/p1/p2/p3/p4`**（原型代码未重排，只能照拍现状；decisions.md"按重排推进"作废）。
