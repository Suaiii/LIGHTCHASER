...
> **更关键——根因重定义**：负责人明确"**GL 现在几乎没有任何问题了**"；楼群消失的**根因不是 GL 本身，是我们中间叠加的大量渲染工作**。解法＝**把附加渲染全部剥离、回到原生 GL 3D 模型即可**（做减法），**不是继续做"捕获消失 + 动画恢复"的攻坚**。本任务因此从"bug 攻坚"转为"**渲染回退（subtract-to-vanilla）**"——目标是用原生 GL 3D 模型稳定呈现，而非层层加 recovery 逻辑。
> **定位**：2D（HERMES-10）是拿得稳的招牌底；GL 作差异化上探（3D 光影是 Campus 没有的锋刃），**稳了就加、不稳也不塌招牌**。下方 §0 起原文的"关键路径命门"口径按本横幅降级重读。
>
...
- **受众**：AI 编码代理
- **状态**：🔻降 P1 差异化上探（7.19）；方向＝剥离附加渲染回原生 GL 3D 模型（做减法）
- **优先级**：**🔻P1（差异化上探，非关键路径）**
